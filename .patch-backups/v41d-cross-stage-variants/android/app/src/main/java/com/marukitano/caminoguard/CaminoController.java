...
public final class CaminoController {
    ...
    private boolean handleStagePlaceTap(
        ...
        /*
         * finite stage variant carousel:
         *
         *   tap 1 -> primary a
         *   tap 2 -> b
         *   tap 3 -> c
         *   ...
         *   next  -> leave stage mode and let the ordinary Camino tap run.
         *
         * Returning false is intentional. attachMap() then clears the stage
         * visual and forwards this very same tap to CaminoSelectionController,
         * whose existing Tap-3 semantics start a fresh blue-marker selection.
         */
        if (sameStage
    ...
    private List<StageRouteSelection> findOutgoingStages(
        ...
        for (RouteTrack candidate
            ...
            /*
             * Do not require candidate.fromKey == placeKey here.
             *
             * Some official CNIG variant files describe only the branch from
             * the stage start to a later rejoin point and their semantic endpoint
             * metadata is not guaranteed to mirror the primary a-track.
             *
             * Geometry is authoritative for this interaction: a usable outgoing
             * variant of the same stage number must physically touch the tapped
             * stage shell at one of its two endpoints.
             */
            LatLng candidateFirst =
                    candidate.points.get(
                            0
                    );

            LatLng candidateLast =
                    candidate.points.get(
                            candidate.points.size() - 1
                    );

            double candidateStartDistanceM =
                    Math.min(
            ...
            if (candidateStartDistanceM > 750.0) {
                continue;
            }
